...
static ssize_t gt5688_tool_read(struct file *filp, char __user *buffer, size_t count, loff_t *ppos);
static ssize_t gt5688_tool_write(struct file *filp, const char *buffer, size_t count, loff_t *ppos);


static int gt5688_tool_release(struct inode *inode, struct file *filp);
static int gt5688_tool_open(struct inode *inode, struct file *file);
...
static struct proc_dir_entry *gt5688_tool_proc_entry;
static struct file_operations gt5688_tool_fops = {
	.read = gt5688_tool_read,
	.write = gt5688_tool_write,
  .open = gt5688_tool_open,
  .release = gt5688_tool_release,
  .owner = THIS_MODULE,
};
...
int gt5688_init_tool_node(void)
{
	...
	gt5688_tool_proc_entry = proc_create(procname, 0755, NULL, &gt5688_tool_fops);
	if (gt5688_tool_proc_entry == NULL) {
...
void gt5688_deinit_tool_node(void)
{
...
static s32 tool_i2c_read(u8 *buf, u16 len)
{
	...
	if (!gt5688_i2c_read(addr, &buf[2], len)) {
		return 1;
	}
	return -1;
}
...
{
	u16 addr = (buf[0] << 8) + buf[1];
	if (!gt5688_i2c_write(addr, &buf[2], len - 2)) {
		return 1;
	}
	return -1;
}
...
Output:
    Return write length.
********************************************************/
static ssize_t gt5688_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	u64 ret = 0;
	GTP_DEBUG_FUNC();
	...
	if (1 == cmd_head.wr) {
	...
	} else if (3 == cmd_head.wr) {	/* gt5688 unused */
		...
		if (cmd_head.data[GTP_ADDR_LENGTH]) {
			...
			gt5688_rawdiff_mode = true;
		} else {
			gt5688_rawdiff_mode = false;
		...
		thrd = kthread_run(gt5688_update_firmware, (void *)cmd_head.data, "GT1x FW Update");
		if (IS_ERR(thrd)) {
			return PTR_ERR(thrd);
		}
	}
...
static ssize_t gt5688_tool_read(struct file *filp, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		buffer[0] = gt5688_update_info.progress >> 8;
		buffer[1] = gt5688_update_info.progress & 0xff;
		buffer[2] = gt5688_update_info.max_progress >> 8;
		buffer[3] = gt5688_update_info.max_progress & 0xff;
		*ppos += 4;
		return 4;
