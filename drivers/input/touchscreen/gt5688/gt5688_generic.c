/* drivers/input/touchscreen/gt5688_generic.c
 *
...
/*#include "gt5688_tpd_custom.h"*/
#include "gt5688.h"
#include "gt5688_generic.h"
#include "gt5688_cfg.h"
...
/*******************GLOBAL VARIABLE*********************/
struct i2c_client *gt5688_i2c_client;
static struct workqueue_struct *gt5688_workqueue;

u8 gt5688_config[GTP_CONFIG_MAX_LENGTH] = { 0 };
u32 gt5688_cfg_length = GTP_CONFIG_MAX_LENGTH;

CHIP_TYPE_T gt5688_chip_type = CHIP_TYPE_NONE;
struct gt5688_version_info gt5688_version = {
	.product_id = {0},
	.patch_id = 0,
	.mask_id = 0,
	.sensor_id = 0,
...
const u16 gt5688_touch_key_array[] = GTP_KEY_TAB;
...
static const u16 gt5688_stylus_key_array[] = GTP_STYLUS_KEY_TAB;
#endif

#define GOODIX_SYSFS_DIR      "goodix"
static struct kobject *sysfs_rootdir;

volatile int gt5688_rawdiff_mode;
u8 gt5688_wakeup_level;
u8 gt5688_init_failed;
u8 gt5688_int_type;
u32 gt5688_abs_x_max;
u32 gt5688_abs_y_max;
int gt5688_halt;
bool gt5688_ics_slot_report;

#if GTP_DEBUG_NODE
static ssize_t gt5688_debug_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gt5688_debug_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *gt5688_debug_proc_entry;
static const struct file_operations gt5688_debug_fops = {
	.owner = THIS_MODULE,
	.read = gt5688_debug_read_proc,
	.write = gt5688_debug_write_proc,
};

static s32 gt5688_init_debug_node(void)
{
	gt5688_debug_proc_entry = proc_create(GT5688_DEBUG_PROC_FILE, 0660, NULL, &gt5688_debug_fops);
	if (gt5688_debug_proc_entry == NULL) {
		GTP_ERROR("Create proc entry /proc/%s FAILED!", GT5688_DEBUG_PROC_FILE);
		return -1;
	}
	GTP_INFO("Created proc entry /proc/%s.", GT5688_DEBUG_PROC_FILE);
	return 0;
}

static void gt5688_deinit_debug_node(void)
{
	if (gt5688_debug_proc_entry != NULL) {
		remove_proc_entry(GT5688_DEBUG_PROC_FILE, NULL);
	}
}

static ssize_t gt5688_debug_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	char *ptr = page;
	...
	ptr += sprintf(ptr, "==== GT5688 default config setting in driver====\n");

	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		ptr += sprintf(ptr, "0x%02X,", gt5688_config[i]);
		if (i % 10 == 9)
			ptr += sprintf(ptr, "\n");
	}
	...
	ptr += sprintf(ptr, "==== GT5688 config read from chip====\n");
	i = gt5688_i2c_read(GTP_REG_CONFIG_DATA, temp_data, GTP_CONFIG_MAX_LENGTH);
	...
	ptr += sprintf(ptr, "\n");
	/* Touch PID & VID */
	ptr += sprintf(ptr, "==== GT5688 Version Info ====\n");

	gt5688_i2c_read(GTP_REG_VERSION, temp_data, 12);
...
static ssize_t gt5688_debug_write_proc(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	s32 ret = 0;
	...
	/*send config*/
	if (count == gt5688_cfg_length) {
		memcpy(gt5688_config, buf, count);
		ret = gt5688_send_cfg(gt5688_config, gt5688_cfg_length);
		if (ret < 0) {
			GTP_ERROR("send gt5688_config failed.");
			return -EFAULT;
		}
		gt5688_abs_x_max = (gt5688_config[RESOLUTION_LOC + 1] << 8) + gt5688_config[RESOLUTION_LOC];
		gt5688_abs_y_max = (gt5688_config[RESOLUTION_LOC + 3] << 8) + gt5688_config[RESOLUTION_LOC + 2];
	...
	/*force clear gt5688_config*/
	if (strcmp(mode_str, "clear_config") == 0) {
		GTP_INFO("Force clear gt5688_config");
		gt5688_send_cmd(GTP_CMD_CLEAR_CFG, 0);
		return count;
	}
	...
	if (strcmp(mode_str, "poweron") == 0) {
		gt5688_power_switch(1);
		return count;
	}

	if (strcmp(mode_str, "poweroff") == 0) {
		gt5688_power_switch(0);
		return count;
	}

	if (strcmp(mode_str, "version") == 0) {
		gt5688_read_version(NULL);
		return count;
	}
	...
	if (strcmp(mode_str, "charger") == 0) {
		gt5688_charger_config(mode);
		return count;
	}
#endif
	sscanf(buf, "%s %s", (char *)&mode_str, (char *)&arg1);
	if (strcmp(mode_str, "update") == 0) {
		gt5688_update_firmware(arg1);
		return count;
	}

	if (strcmp(mode_str, "sendconfig") == 0) {
		cfg_len = gt5688_parse_config(arg1, temp_config);
		if (cfg_len < 0) {
			return -1;
		}
		gt5688_send_cfg(temp_config, gt5688_cfg_length);
		return count;
	}

	if (strcmp(mode_str, "debug_gesture") == 0) {
#if GTP_GESTURE_WAKEUP
		gt5688_gesture_debug(!!mode);
#endif
	}

	if (strcmp(mode_str, "force_update") == 0) {
		gt5688_update_info.force_update = !!mode;
	}
	return gt5688_debug_proc(buf, count);
}
#endif
...
int gt5688_parse_config(char *filename, u8 *config)
{
	mm_segment_t old_fs;
	struct file *fp = NULL;
	u8 *buf;
...
s32 gt5688_do_i2c_read(struct i2c_msg *msgs, u16 addr, u8 *buffer, s32 len)
{
	s32 ret = -1;
	s32 pos = 0;
	s32 data_length = len;
	...
	while (pos != data_length) {
		...
		ret = i2c_transfer(gt5688_i2c_client->adapter, msgs, 2);
		if (ret != 2) {
...
s32 gt5688_do_i2c_write(struct i2c_msg *msg, u16 addr, u8 *buffer, s32 len)
{
	s32 ret = -1;
	s32 pos = 0;
	s32 data_length = len;
	...
	while (pos != data_length) {
		...
		ret = i2c_transfer(gt5688_i2c_client->adapter, msg, 1);
		if (ret != 1) {
			GTP_ERROR("I2c transfer error! (%d)", ret);
			kfree(data);
			return ERROR_IIC;
...
#if !GTP_ESD_PROTECT
static s32 gt5688_i2c_test(void)
{
	u8 retry = 0;
	s32 ret = -1;
	u32 hw_info = 0;
	...
	while (retry++ < 3) {
		ret = gt5688_i2c_read(GTP_REG_HW_INFO, (u8 *) &hw_info, sizeof(hw_info));
		if (!ret) {
...
/**
 * gt5688_i2c_read_dbl_check - read twice and double check
 * @addr: register address
 * @buffer: data buffer
 * @len: bytes to read
 * Return    <0: i2c error, 0: ok, 1:fail
 */
s32 gt5688_i2c_read_dbl_check(u16 addr, u8 *buffer, s32 len)
{
	u8 buf[16] = { 0 };
	u8 confirm_buf[16] = { 0 };
	int ret;
	...
	ret = gt5688_i2c_read(addr, buf, len);
	if (ret < 0) {
		return ret;
	}
	...
	ret = gt5688_i2c_read(addr, confirm_buf, len);
	if (ret < 0) {
		return ret;
	}
...
s32 gt5688_send_cfg(u8 *config, int cfg_len)
{
	...
	while (retry++ < 5) {
		ret = gt5688_i2c_write(GTP_REG_CONFIG_DATA, config, cfg_len);
		if (!ret) {
...
/**
 * gt5688_init_panel - Prepare config data for touch ic, don't call this function
 * after initialization.
 *
 * Return 0--success,<0 --fail.
 */
s32 gt5688_init_panel(void)
{
	s32 ret = 0;
	u8 cfg_len = 0;
	...
	memset(gt5688_config, 0, sizeof(gt5688_config));
	memcpy(gt5688_config, cfgs[sensor_id], cfg_len);

	/* clear the flag, avoid failure when send the_config of driver. */
	gt5688_config[0] &= 0x7F;

#if GTP_CUSTOM_CFG
	gt5688_config[RESOLUTION_LOC] = (u8) GTP_MAX_WIDTH;
	gt5688_config[RESOLUTION_LOC + 1] = (u8) (GTP_MAX_WIDTH >> 8);
	gt5688_config[RESOLUTION_LOC + 2] = (u8) GTP_MAX_HEIGHT;
	gt5688_config[RESOLUTION_LOC + 3] = (u8) (GTP_MAX_HEIGHT >> 8);

	if (GTP_INT_TRIGGER == 0) {	/* RISING  */
		gt5688_config[TRIGGER_LOC] &= 0xfe;
	} else if (GTP_INT_TRIGGER == 1) {	/* FALLING */
		gt5688_config[TRIGGER_LOC] |= 0x01;
	}
	set_reg_bit(gt5688_config[MODULE_SWITCH3_LOC], 5, !gt5688_wakeup_level);
#endif /* END GTP_CUSTOM_CFG */

#else /* DRIVER NOT SEND CONFIG */
	cfg_len = GTP_CONFIG_MAX_LENGTH;
	ret = gt5688_i2c_read(GTP_REG_CONFIG_DATA, gt5688_config, cfg_len);
	if (ret < 0) {
		return ret;
	}
	...
	GTP_DEBUG_FUNC();
	/* match resolution when gt5688_abs_x_max & gt5688_abs_y_max have been set already */
	if ((gt5688_abs_x_max == 0) && (gt5688_abs_y_max == 0)) {
		gt5688_abs_x_max = (gt5688_config[RESOLUTION_LOC + 1] << 8) + gt5688_config[RESOLUTION_LOC];
		gt5688_abs_y_max = (gt5688_config[RESOLUTION_LOC + 3] << 8) + gt5688_config[RESOLUTION_LOC + 2];
		gt5688_int_type = (gt5688_config[TRIGGER_LOC]) & 0x03;
		gt5688_wakeup_level = !(gt5688_config[MODULE_SWITCH3_LOC] & 0x20);
	} else {
		gt5688_config[RESOLUTION_LOC] = (u8) gt5688_abs_x_max;
		gt5688_config[RESOLUTION_LOC + 1] = (u8) (gt5688_abs_x_max >> 8);
		gt5688_config[RESOLUTION_LOC + 2] = (u8) gt5688_abs_y_max;
		gt5688_config[RESOLUTION_LOC + 3] = (u8) (gt5688_abs_y_max >> 8);
		set_reg_bit(gt5688_config[MODULE_SWITCH3_LOC], 5, !gt5688_wakeup_level);
		gt5688_config[TRIGGER_LOC] = (gt5688_config[TRIGGER_LOC] & 0xFC) | gt5688_int_type;
	}

	GTP_INFO("X_MAX=%d,Y_MAX=%d,TRIGGER=0x%02x,WAKEUP_LEVEL=%d", gt5688_abs_x_max, gt5688_abs_y_max, gt5688_int_type, gt5688_wakeup_level);

	gt5688_cfg_length = cfg_len;
	ret = gt5688_send_cfg(gt5688_config, gt5688_cfg_length);
	return ret;
}

void gt5688_select_addr(void)
{
	GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);
	GTP_GPIO_OUTPUT(GTP_INT_PORT, gt5688_i2c_client->addr == 0x14);
	usleep_range(2000, 3000);
	GTP_GPIO_OUTPUT(GTP_RST_PORT, 1);
	usleep_range(2000, 3000);
}

static s32 gt5688_set_reset_status(void)
{
	/* 0x8040 ~ 0x8043 */
	...
	ret = gt5688_i2c_write(GTP_REG_CMD + 1, &value[1], 3);
	if (ret < 0)
		return ret;

	return gt5688_i2c_write(GTP_REG_CMD, value, 1);
}

#if GTP_INCELL_PANEL
int gt5688_write_and_readback(u16 addr, u8 *buffer, s32 len)
{
	int ret;
	u8 d[len];

	ret = gt5688_i2c_write(addr, buffer, len);
	if (ret < 0)
		return -1;

	ret = gt5688_i2c_read(addr, d, len);
...
#define RST_RETRY       5
	...
	do {
		...
		/* test i2c */
		ret = gt5688_i2c_read(0x4220, d, 1);
	...
	do {
		d[0] = 0x0C;
		ret = gt5688_write_and_readback(0x4180, d, 1);
	...
	do {
		d[0] = 0x00;
		ret = gt5688_write_and_readback(0x4305, d, 1);
		if (ret < 0)
			continue;

		d[0] = 0x2B;
		d[1] = 0x24;
		ret = gt5688_write_and_readback(0x42c4, d, 2);
		if (ret < 0)
			continue;

		d[0] = 0xE1;
		d[1] = 0xD3;
		ret = gt5688_write_and_readback(0x42e4, d, 2);
		if (ret < 0)
			continue;
		d[0] = 0x01;
		ret = gt5688_write_and_readback(0x4305, d, 1);
...
s32 gt5688_reset_guitar(void)
{
	...
	ret = gt5688_set_reset_status();
	return ret;
}

/**
 * gt5688_read_version - Read gt5688 version info.
 * @ver_info: address to store version info
 * Return 0-succeed.
 */
s32 gt5688_read_version(struct gt5688_version_info *ver_info)
{
	s32 ret = -1;
	u8 buf[12] = { 0 };
	u32 mask_id = 0;
	...
	while (retry--) {
		ret = gt5688_i2c_read_dbl_check(GTP_REG_VERSION, buf, sizeof(buf));
		if (!ret) {
			checksum = 0;
...
/**
 * gt5688_get_chip_type - get chip type .
 *
 * different chip synchronize in different way,
 */
s32 gt5688_get_chip_type(void)
{
	u8 opr_buf[4] = { 0x00 };
	u8 gt5688_data[] = { 0x02, 0x08, 0x90, 0x00 };
	u8 gt9l_data[] = { 0x03, 0x10, 0x90, 0x00 };
	s32 ret = -1;

	/* chip type already exist */
	if (gt5688_chip_type != CHIP_TYPE_NONE) {
		return 0;
	}

	/* read hardware */
	ret = gt5688_i2c_read_dbl_check(GTP_REG_HW_INFO, opr_buf, sizeof(opr_buf));
	if (ret) {
	...
	/* find chip type */
	if (!memcmp(opr_buf, gt5688_data, sizeof(gt5688_data))) {
		gt5688_chip_type = CHIP_TYPE_GT5688;
	} else if (!memcmp(opr_buf, gt9l_data, sizeof(gt9l_data))) {
		gt5688_chip_type = CHIP_TYPE_GT2X;
	}

	if (gt5688_chip_type != CHIP_TYPE_NONE) {
		GTP_INFO("Chip Type: %s", (gt5688_chip_type == CHIP_TYPE_GT5688) ? "GT5688" : "GT2X");
		return 0;
	} else {
		return -1;
	}
...
/**
 * gt5688_enter_sleep - Eter sleep function.
 *
 * Returns  0--success,non-0--fail.
 */
static s32 gt5688_enter_sleep(void)
{
	s32 retry = 0;

#if GTP_POWER_CTRL_SLEEP
	if (!gt5688_power_switch(SWITCH_OFF)) {
		GTP_INFO("Enter sleep mode by poweroff");
		return 0;
	}
#endif

	if (gt5688_wakeup_level == 1) {	/* high level wakeup */
		GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);
	}
	usleep_range(5000, 6000);

	while (retry++ < 3) {
		if (!gt5688_send_cmd(GTP_CMD_SLEEP, 0)) {
...
/**
 * gt5688_wakeup_sleep - wakeup from sleep mode Function.
 *
 * Return: 0--success,non-0--fail.
 */
static s32 gt5688_wakeup_sleep(void)
{
	u8 retry = 0;
	s32 ret = -1;
	int flag = 0;
	...
	if (!gt5688_power_switch(SWITCH_ON)) {
		gt5688_power_reset();
	...
	while (retry++ < 2) {
...
#endif
		{
			...
			GTP_GPIO_OUTPUT(GTP_INT_PORT, gt5688_wakeup_level);
			usleep_range(5000, 6000);
 ...
 */
s32 gt5688_send_cmd(u8 cmd, u8 data)
{
	s32 ret;
	...
	ret = gt5688_i2c_write(GTP_REG_CMD + 1, &buffer[1], 2);
	ret |= gt5688_i2c_write(GTP_REG_CMD, &buffer[0], 1);
	msleep(50);
...
void gt5688_power_reset(void)
{
	static int rst_flag;
	s32 i = 0;

	if (rst_flag || gt5688_update_info.status) {
		return;
	}
	GTP_INFO("force_reset_guitar");
	rst_flag = 1;
	gt5688_irq_disable();
	gt5688_power_switch(SWITCH_OFF);
	msleep(30);
	gt5688_power_switch(SWITCH_ON);
	msleep(30);

	for (i = 0; i < 5; i++) {
		if (gt5688_reset_guitar()) {
			continue;
		}
		if (gt5688_send_cfg(gt5688_config, gt5688_cfg_length)) {
			msleep(500);
			continue;
		}
		break;
...
s32 gt5688_request_event_handler(void)
{
	s32 ret = -1;
	u8 rqst_data = 0;

	ret = gt5688_i2c_read(GTP_REG_RQST, &rqst_data, 1);
	if (ret) {
	...
	case GTP_RQST_CONFIG:
		...
		ret = gt5688_send_cfg(gt5688_config, gt5688_cfg_length);
		if (ret) {
			GTP_ERROR("Send gt5688_config error.");
		} else {
			GTP_INFO("Send gt5688_config success.");
			rqst_data = GTP_RQST_RESPONDED;
			gt5688_i2c_write(GTP_REG_RQST, &rqst_data, 1);
		}
		break;
	case GTP_RQST_RESET:
...
/**
 * gt5688_touch_event_handler - handle touch event
 * (pen event, key event, finger touch envent)
 * @data:
 * Return    <0: failed, 0: succeed
 */
s32 gt5688_touch_event_handler(u8 *data, struct input_dev *dev, struct input_dev *pen_dev)
{
	 ...
	 */
	if (touch_num > 1) {
		ret = gt5688_i2c_read((GTP_READ_COOR_ADDR + 11), &touch_data[11], 1 + 8 * touch_num + 2 - 11);
		if (ret) {
			return ret;
		}
	}
	...
	if (check_sum) { /* checksum error*/
		ret = gt5688_i2c_read(GTP_READ_COOR_ADDR, touch_data, 3 + 8 * touch_num);
		if (ret) {
			return ret;
		}
	...
	if (CHK_BIT(cur_event, BIT_STYLUS_KEY) || CHK_BIT(pre_event, BIT_STYLUS_KEY)) {
		...
		for (i = 4; i < 6; i++) {
			input_report_key(pen_dev, gt5688_stylus_key_array[i - 4], temp & (0x01 << i));
		}
	...
	if (CHK_BIT(cur_event, BIT_STYLUS)) {
		...
		input_x = GTP_WARP_X(gt5688_abs_x_max, input_x);
		input_y = GTP_WARP_Y(gt5688_abs_y_max, input_y);

		GTP_DEBUG("Pen touch DOWN.");
		gt5688_pen_down(input_x, input_y, input_w, 0);
	...
	if (CHK_BIT(cur_event, BIT_TOUCH_KEY) || CHK_BIT(pre_event, BIT_TOUCH_KEY)) {
		for (i = 0; i < GTP_MAX_KEY_NUM; i++) {
			input_report_key(dev, gt5688_touch_key_array[i], key_value & (0x01 << i));
		}
	...
	if (CHK_BIT(cur_event, BIT_TOUCH_KEY) || CHK_BIT(pre_event, BIT_TOUCH_KEY)) {
		for (i = 0; i < TPD_KEY_COUNT; i++) {
			if (key_value & (0x01 << i)) {
				gt5688_touch_down(tpd_virtual_key_array[i].x, tpd_virtual_key_array[i].y, 0, 0);
				GTP_DEBUG("Key Down.");
				break;
			}
		}
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				gt5688_touch_down(input_x, input_y, input_w, i);
...
static void gt5688_pen_init(void)
{
	s32 ret = 0;
	...
	input_set_abs_params(pen_dev, ABS_MT_POSITION_X, 0, gt5688_abs_x_max, 0, 0);
	input_set_abs_params(pen_dev, ABS_MT_POSITION_Y, 0, gt5688_abs_y_max, 0, 0);
...
void gt5688_pen_down(s32 x, s32 y, s32 size, s32 id)
{
...
static struct gt5688_ps_device *gt5688_ps_dev;

static void gt5688_ps_report(int state)
{
#ifdef PLATFORM_MTK
	s32 ret = -1;
...
#else
	input_report_abs(gt5688_ps_dev->input_dev, ABS_DISTANCE, !!state);
	input_sync(gt5688_ps_dev->input_dev);
#endif /* End PLATFROM_MTK */

	GTP_INFO("Report proximity state: %s", state == PS_FARAWAY ? "FARAWAY":"NEAR");
}

static s32 gt5688_ps_enable(s32 enable)
{
	u8 state;
	s32 ret = -1;
	...
	state = enable ? 1 : 0;
	if (gt5688_chip_type == CHIP_TYPE_GT5688)
		ret = gt5688_i2c_write(GTP_REG_PROXIMITY_ENABLE, &state, 1);
	else if (gt5688_chip_type == CHIP_TYPE_GT2X)
		ret = gt5688_send_cmd(state ? 0x12 : 0x13, 0);
	if (ret) {
		GTP_ERROR("GTP %s proximity cmd failed.", state ? "enable" : "disable");
	}

	if (!ret && enable) {
		gt5688_ps_dev->enabled = 1;
	} else {
		gt5688_ps_dev->enabled = 0;
	}
	gt5688_ps_dev->state = PS_FARAWAY;
	GTP_INFO("Proximity function %s %s.", state ? "enable" : "disable", ret ? "fail" : "success");
	return ret;
}

int gt5688_prox_event_handler(u8 *data)
{
	u8 ps = 0;

	if (gt5688_ps_dev && gt5688_ps_dev->enabled) {
		ps = (data[0] & 0x60) ? 0 : 1;
		if (ps != gt5688_ps_dev->state) {
			gt5688_ps_report(ps);
			gt5688_ps_dev->state = ps;
...
#ifdef PLATFORM_MTK
static inline s32 gt5688_get_ps_value(void)
{
	return gt5688_ps_dev->state;
}

static s32 gt5688_ps_operate(void *self, u32 command, void *buff_in, s32 size_in, void *buff_out, s32 size_out, s32 *actualout)
{
	s32 err = 0;
	s32 value;
	hwm_sensor_data *sensor_data;
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			...
			sensor_data->values[0] = gt5688_get_ps_value();
			sensor_data->value_divide = 1;
			sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
		}
...
#ifndef PLATFORM_MTK
static ssize_t gt5688_ps_enable_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d", gt5688_ps_dev->enabled);
}

static ssize_t gt5688_ps_enable_store(struct kobject *kobj, struct kobj_attribute *attr,
...
static ssize_t gt5688_ps_state_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d", gt5688_ps_dev->state);
}

static ssize_t gt5688_ps_state_store(struct kobject *kobj, struct kobj_attribute *attr,
		const char *buf, size_t count)
{
	unsigned int input;
	...
	if (input == 1) {
		...
		gt5688_ps_dev->state = PS_NEAR;
	} else {
		return -EINVAL;
	}

	gt5688_ps_report(gt5688_ps_dev->state);
	return count;
}

static struct kobj_attribute ps_attrs[] = {
	__ATTR(enable, S_IWUGO | S_IRUGO, gt5688_ps_enable_show, gt5688_ps_enable_store),
	__ATTR(state, S_IWUGO | S_IRUGO, gt5688_ps_state_show, gt5688_ps_state_store)
};

#endif /* End PLATFORM_MTK */

static int gt5688_ps_init(void)
{
	int err;

	gt5688_ps_dev = kzalloc(sizeof(struct gt5688_ps_device), GFP_KERNEL);
	if (!gt5688_ps_dev) {
		return  -ENOMEM;
	}
...
#ifdef PLATFORM_MTK
	gt5688_ps_dev->obj_ps.polling = 0;	/* 0--interrupt mode;1--polling mode; */
	gt5688_ps_dev->obj_ps.sensor_operate = gt5688_ps_operate;

	if (hwmsen_attach(ID_PROXIMITY, &gt5688_ps_dev->obj_ps)) {
	...
	return 0;
#else
	gt5688_ps_dev->input_dev = input_allocate_device();
	if (!gt5688_ps_dev->input_dev) {
		GTP_ERROR("Failed to alloc inpput device for proximity!");
		err = -ENOMEM;
		goto err_exit;
	}

	gt5688_ps_dev->input_dev->name = GTP_PS_DEV_NAME;
	gt5688_ps_dev->input_dev->phys = "goodix/proximity";
	gt5688_ps_dev->input_dev->id.bustype = BUS_I2C;
	gt5688_ps_dev->input_dev->id.vendor = 0xDEED;
	gt5688_ps_dev->input_dev->id.product = 0xBEEF;
	gt5688_ps_dev->input_dev->id.version = 1;
	set_bit(EV_ABS, gt5688_ps_dev->input_dev->evbit);
	input_set_abs_params(gt5688_ps_dev->input_dev, ABS_DISTANCE, 0, 1, 0, 0);

	err = input_register_device(gt5688_ps_dev->input_dev);
	if (err) {
		GTP_ERROR("Failed to register proximity input device: %s!", gt5688_ps_dev->input_dev->name);
	...
	gt5688_ps_dev->kobj = kobject_create_and_add("proximity", sysfs_rootdir);
	if (!gt5688_ps_dev->kobj) {
	...
	/*create sysfs files*/
	{
		...
		for (i = 0; i < sizeof(ps_attrs)/sizeof(ps_attrs[0]); i++) {
			if (sysfs_create_file(gt5688_ps_dev->kobj, &ps_attrs[i].attr)) {
	...
	return 0;
err_create_file:
	kobject_put(gt5688_ps_dev->kobj);
err_register_dev:
	input_free_device(gt5688_ps_dev->input_dev);
#endif  /* End PLATFROM_MTK */
...
static void gt5688_ps_deinit(void)
{
	if (gt5688_ps_dev) {
#ifndef PLATFORM_MTK
		int i = 0;
		for (; i < sizeof(ps_attrs) / sizeof(ps_attrs[0]); i++) {
			sysfs_remove_file(gt5688_ps_dev->kobj, &ps_attrs[i].attr);
		}
		kobject_del(gt5688_ps_dev->kobj);
		input_free_device(gt5688_ps_dev->input_dev);
#endif
		kfree(gt5688_ps_dev);
	}
...
static void gt5688_esd_check_func(struct work_struct *);

void gt5688_init_esd_protect(void)
{
	...
	INIT_DELAYED_WORK(&esd_check_work, gt5688_esd_check_func);
	mutex_init(&esd_lock);
}

static void gt5688_deinit_esd_protect(void)
{
	gt5688_esd_switch(SWITCH_OFF);
}

void gt5688_esd_switch(s32 on)
{
	mutex_lock(&esd_lock);
	if (SWITCH_ON == on) {	/* switch on esd check */
		if (!esd_running) {
			...
			queue_delayed_work(gt5688_workqueue, &esd_check_work, esd_work_cycle);
		}
...
static void gt5688_esd_check_func(struct work_struct *work)
{
	s32 i = 0;
	s32 ret = -1;
	u8 esd_buf[4] = { 0 };
	...
	for (i = 0; i < 3; i++) {
		ret = gt5688_i2c_read(GTP_REG_CMD, esd_buf, 4);
...
static struct smart_cover_device *gt5688_sc_dev;

/**
 * gt5688_smart_cover_update_state - update smart cover config
 */
static int gt5688_smart_cover_update_state(void)
{
	int ret = 0;
	struct smart_cover_device *dev = gt5688_sc_dev;
	...
	if (!dev->suspended) {
		if (dev->state) {  /* near */
			ret = gt5688_send_cfg(dev->config, dev->cfg_len);
		} else {
#if GTP_CHARGER_SWITCH
			gt5688_charger_config(1);  /*charger detector module check and*/
			/*send a config*/
#else
			ret = gt5688_send_cfg(gt5688_config, gt5688_cfg_length);
#endif
...
static ssize_t smart_cover_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	dev->state = s;
	gt5688_smart_cover_update_state();

	return count;
}

/**
 * gt5688_parse_sc_cfg - parse smart cover config
 * @sensor_id: sensor id of the hardware
 */
int gt5688_parse_sc_cfg(int sensor_id)
{
#undef _cfg_array_
#define _cfg_array_(n)   GTP_SMART_COVER_CFG_GROUP##n
	...
	if (!gt5688_sc_dev)
		return -ENODEV;
	cfg = gt5688_sc_dev->config;
	len = &gt5688_sc_dev->cfg_len;
	...
	do {
		...
		*len = cfg_lens[sensor_id];
		if (*len == 0 || *len != gt5688_cfg_length) {
		...
		cfg[0] &= 0x7F;
		set_reg_bit(cfg[TRIGGER_LOC], 0, gt5688_int_type);
		set_reg_bit(cfg[MODULE_SWITCH3_LOC], 5, !gt5688_wakeup_level);
	} while (0);
#endif
	return 0;
}
...
static int gt5688_smart_cover_init(void)
{
	int err = 0;

	gt5688_sc_dev = kzalloc(sizeof(struct smart_cover_device), GFP_KERNEL);
	if (!gt5688_sc_dev) {
		GTP_ERROR("SmartCover init failed in step: 1.");
		return -ENOMEM;
	}

	gt5688_sc_dev->enabled = 1;
	gt5688_parse_sc_cfg(gt5688_version.sensor_id);
	...
	if (!gt5688_sc_dev->kobj)
		gt5688_sc_dev->kobj = kobject_create_and_add("smartcover", sysfs_rootdir);
	if (!gt5688_sc_dev->kobj) {
		err = -3;
		goto exit_free_mem;
	}

	if (sysfs_create_file(gt5688_sc_dev->kobj, &sc_attr.attr)) {
		err = -4;
		goto exit_put_kobj;
	}
	...
	return 0;
exit_put_kobj:
	kobject_put(gt5688_sc_dev->kobj);
exit_free_mem:
	kfree(gt5688_sc_dev);
	gt5688_sc_dev = NULL;
...
static void gt5688_smart_cover_deinit(void)
{
	if (!gt5688_sc_dev) {
		return;
	}

	kobject_del(gt5688_sc_dev->kobj);
	kfree(gt5688_sc_dev);
	gt5688_sc_dev = NULL;
}
#endif
 ...
 */
#if GTP_CHARGER_SWITCH
static u8 gt5688_config_charger[GTP_CONFIG_MAX_LENGTH] = { 0 };
...
static void gt5688_charger_work_func(struct work_struct *);

/**
 * gt5688_parse_chr_cfg - parse  charger config
 * @sensor_id: sensor id of the hardware
 * Return:  0: succeed, <0 error
 */
int gt5688_parse_chr_cfg(int sensor_id)
{
#undef _cfg_array_
#define _cfg_array_(n)   GTP_CHARGER_CFG_GROUP##n
	u8 *cfg;
	int len;
	cfg = gt5688_config_charger;
	...
	do {
		...
		cfg[0] &= 0x7F;
		cfg[RESOLUTION_LOC] = (u8) gt5688_abs_x_max;
		cfg[RESOLUTION_LOC + 1] = (u8) (gt5688_abs_x_max >> 8);
		cfg[RESOLUTION_LOC + 2] = (u8) gt5688_abs_y_max;
		cfg[RESOLUTION_LOC + 3] = (u8) (gt5688_abs_y_max >> 8);
...
static void gt5688_init_charger(void)
{
	...
	INIT_DELAYED_WORK(&charger_switch_work, gt5688_charger_work_func);
	spin_lock_init(&charger_lock);

	if (gt5688_parse_chr_cfg(gt5688_version.sensor_id) < 0) {
		GTP_ERROR("Error occured when parse charger config.");
	}
}

/**
 * gt5688_charger_switch - switch states of charging work thread
 *
 * @on: SWITCH_ON - start work thread, SWITCH_OFF: stop .
 */
void gt5688_charger_switch(s32 on)
{
	...
	if (SWITCH_ON == on) {
		if (!charger_running) {
			...
			queue_delayed_work(gt5688_workqueue, &charger_switch_work, charger_work_cycle);
		} else {
			spin_unlock(&charger_lock);
		}
	} else {
...
/**
 * gt5688_charger_config - check and update charging status configuration
 * @dir_update
 * 	 0: check before send charging status configuration
 *  	 1: directly send charging status configuration
 *
 */
void gt5688_charger_config(s32 dir_update)
{
	static u8 chr_pluggedin;

#if GTP_SMART_COVER
	if (gt5688_sc_dev && gt5688_sc_dev->enabled
			&& gt5688_sc_dev->state) {
		return;
	}
#endif

	if (gt5688_get_charger_status()) {
		if (!chr_pluggedin || dir_update) {
			GTP_INFO("Charger Plugin.");
			if (gt5688_send_cfg(gt5688_config_charger, gt5688_cfg_length)) {
				GTP_ERROR("Send config for Charger Plugin failed!");
			}
			if (gt5688_send_cmd(GTP_CMD_CHARGER_ON, 0)) {
		...
		if (chr_pluggedin || dir_update) {
			...
			if (gt5688_send_cmd(GTP_CMD_CHARGER_OFF, 0)) {
...
static void gt5688_charger_work_func(struct work_struct *work)
{
	if (!charger_running) {
...
#else
		ret = gt5688_i2c_read_dbl_check(GTP_REG_HN_PAIRED, buf, sizeof(buf));
...
#endif
#if GTP_CHARGER_SWITCH
	gt5688_charger_switch(SWITCH_OFF);
#endif
	gt5688_irq_disable();
...
int gt5688_resume(void)
{
	...
	if (!gesture_enabled && !gt5688_halt)
		return 0;
#else
	if (!gt5688_halt)
		return 0;
	...
	ret = gt5688_wakeup_sleep();
	if (ret < 0) {
		GTP_ERROR("Resume failed.");
	}
#if GTP_HOTKNOT
	if (!hotknot_enabled) {
		gt5688_send_cmd(GTP_CMD_HN_EXIT_SLAVE, 0);
	}
#endif

#if GTP_CHARGER_SWITCH
	gt5688_charger_config(0);
	gt5688_charger_switch(SWITCH_ON);
#endif
...
s32 gt5688_init(void)
{
	...
	while (retry++ < 5) {
		...
		ret = gt5688_i2c_read_dbl_check(GTP_REG_FW_CHK_MAINSYS, reg_val, 1);
		if (ret != 0) {
			continue;
		} else if (reg_val[0] != 0xBE) {
			GTP_ERROR("Check main system not pass[0x%2X].", reg_val[0]);
			gt5688_init_failed = 1;
		}

#if !GTP_AUTO_UPDATE
		/* debug info  */
		ret = gt5688_i2c_read_dbl_check(GTP_REG_FW_CHK_SUBSYS, reg_val, 1);
	...
	ret |= gt5688_init_failed;
	if (ret) {
		GTP_ERROR("Init failed, use default setting");
		gt5688_abs_x_max = GTP_MAX_WIDTH;
		gt5688_abs_y_max = GTP_MAX_HEIGHT;
		gt5688_int_type = GTP_INT_TRIGGER;
		gt5688_wakeup_level = GTP_WAKEUP_LEVEL;
	}

	/* get chip type */
	ret = gt5688_get_chip_type();
	if (ret != 0) {
		GTP_ERROR("Get chip type failed!");
	}

	/* read version information */
	ret = gt5688_read_version(&gt5688_version);
	if (ret != 0) {
		GTP_ERROR("Get verision failed!");
	}

	/* init and send configs */
	ret = gt5688_init_panel();
	if (ret != 0) {
		GTP_ERROR("Init panel failed.");
	}

	gt5688_workqueue = create_singlethread_workqueue("gt5688_workthread");
	if (gt5688_workqueue == NULL) {
		GTP_ERROR("Create workqueue failed!");
	}

	/* init auxiliary  node and functions */
#if GTP_DEBUG_NODE
	gt5688_init_debug_node();
#endif

#if GTP_CREATE_WR_NODE
	gt5688_init_tool_node();
#endif
...
#if GTP_CHARGER_SWITCH
	gt5688_init_charger();
	gt5688_charger_config(1);
	gt5688_charger_switch(SWITCH_ON);
#endif

#if GTP_SMART_COVER
	gt5688_smart_cover_init();
#endif
...
void gt5688_deinit(void)
{
#if GTP_DEBUG_NODE
	gt5688_deinit_debug_node();
#endif
...
#if GTP_CREATE_WR_NODE
	gt5688_deinit_tool_node();
#endif

#if GTP_ESD_PROTECT
	gt5688_deinit_esd_protect();
#endif
	...
	if (gt5688_workqueue) {
		destroy_workqueue(gt5688_workqueue);
	}
