/* drivers/input/touchscreen/gt5688_extents.c
 *
...
static ssize_t gt5688_gesture_data_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	s32 ret = -1;
	GTP_DEBUG("visit gt5688_gesture_data_read. ppos:%d", (int)*ppos);
	if (*ppos) {
		return 0;
	}
	if (size == 4) {
		ret = copy_to_user(((u8 __user *) page), "GT5688", 4);
		return 4;
	}
...
static ssize_t gt5688_gesture_data_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
	s32 ret = 0;
...
s32 gesture_event_handler(struct input_dev *dev)
{
	 ...
	 */
	ret = gt5688_i2c_read(GTP_REG_WAKEUP_GESTURE, doze_buf, 4);
	if (ret < 0) {
		return 0;
	}
	...
	if (extra_len >= 0) {
		...
		ret = gt5688_i2c_read(GTP_REG_WAKEUP_GESTURE + 4,
	...
	if (ges_type == 0 || !QUERYBIT(gestures_flag, ges_type)) {
		...
		doze_buf[0] = 0x00;
		gt5688_i2c_write(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);
		gesture_enter_doze();
		return 0;
	}
	...
	if (len > 0) { /* coor num * 4 + chksum 2*/
		...
		ret = gt5688_i2c_read(GES_BUFFER_ADDR, ges_data, len * 4);
		if (ret < 0) {
		...
		ret = gt5688_i2c_read(0x819F, &ges_data[len * 4], 2);
		if (ret < 0) {
			GTP_ERROR("Read gesture data failed.");
			return 0;
		}
...
void gt5688_gesture_debug(int on)
{
	if (on) {
		gesture_enabled = 1;
...
#define HOTKNOT_VERSION  "GOODIX,GT5688"
u8 hotknot_enabled;
...
static s32 hotknot_enter_transfer_mode(void)
{
	...
	gt5688_irq_disable();
	gt5688_send_cmd(GTP_CMD_HN_TRANSFER, 0);
	msleep(100);
	gt5688_irq_enable();

	ret = gt5688_i2c_read(0x8140, buffer, sizeof(buffer));
	if (ret) {
...
static s32 hotknot_load_authentication_subsystem(void)
{
	s32 ret = 0;
	u8 buffer[5] = { 0 };
	ret = gt5688_hold_ss51_dsp_no_reset();
	if (ret < 0) {
	...
	if (gt5688_chip_type == CHIP_TYPE_GT5688) {
		...
		ret = gt5688_load_patch(gt5688_patch_jump_fw, 4096, 0, 1024 * 8);
		if (ret < 0) {
			GTP_ERROR("Load jump code fail!");
			return ret;
		}
		GTP_INFO("hotknot load auth code.");
		ret = gt5688_load_patch(hotknot_auth_fw, 4096, 4096, 1024 * 8);
		if (ret < 0) {
		...
		GTP_INFO("hotknot load auth code.");
		ret = gt5688_load_patch(hotknot_auth_fw, 4096, 0, 1024 * 6);
		if (ret < 0) {
			GTP_ERROR("load auth system fail!");
			return ret;
		}
	...
	ret = gt5688_startup_patch();
	if (ret < 0) {
		GTP_ERROR("Startup auth system fail!");
		return ret;
	}
	ret = gt5688_i2c_read(GTP_REG_VERSION, buffer, 4);
	if (ret < 0) {
...
s32 hotknot_event_handler(u8 *data)
{
	...
	if (hotknot_paired_flag) {
		s32 ret = -1;
		ret = gt5688_i2c_read(GTP_REG_HN_STATE, hn_state_buf, 6);
		if (ret < 0) {
...
static s32 io_iic_read(u8 *data, void __user *arg)
{
	...
	err = gt5688_i2c_read(addr, &data[CMD_HEAD_LENGTH], data_length);
	if (!err) {
...
static s32 io_iic_write(u8 *data)
{
	...
	err = gt5688_i2c_write(addr, &data[CMD_HEAD_LENGTH], data_length);
	if (!err) {
		err = CMD_HEAD_LENGTH + data_length;
	}
 ...
 *        error otherwise.
 */
static long gt5688_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	u32 value = 0;
	s32 ret = 0;
	u8 *data = NULL;
	...
	while (cnt-- && gt5688_update_info.status) {
		ssleep(1);
	}
...
#ifdef CONFIG_COMPAT
static long gt5688_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static const struct file_operations gt5688_fops = {
	.owner = THIS_MODULE,
#if GTP_GESTURE_WAKEUP
	.read = gt5688_gesture_data_read,
	.write = gt5688_gesture_data_write,
#endif
	.unlocked_ioctl = gt5688_ioctl,
#ifdef CONFIG_COMPAT
    .compat_ioctl = gt5688_compat_ioctl,
#endif
};
...
s32 gt5688_init_node(void)
{
	...
	proc_entry = proc_create(GESTURE_NODE, 0755, NULL, &gt5688_fops);
	if (proc_entry == NULL) {
