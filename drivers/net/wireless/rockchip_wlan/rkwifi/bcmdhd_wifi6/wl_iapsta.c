...
wl_tput_dump(struct wl_apsta_params *apsta_params, struct wl_if_info *cur_if)
{
	...
	if (apsta_params->tput_ts.tv_sec == 0 && apsta_params->tput_ts.tv_nsec == 0) {
		...
		if (ret < 0) {
			IAPSTA_ERROR(cur_if->dev->name, "GET STA INFO failed, %d\n", ret);
			goto error;
		}
		sta = (sta_info_v4_t *)buf;
		if (sta->ver != WL_STA_VER_4 && sta->ver != WL_STA_VER_5) {
			IAPSTA_ERROR(cur_if->dev->name, "GET STA INFO version mismatch\n");
			goto error;
		}
		...
		WL_MSG(cur_if->dev->name, "rssi:%d, tx=%3dMbps(rate:%7d), rx=%3dMbps(rate:%7d)\n",
			dtoh32(scb_val.val), tx_tput, dtoh32(sta->tx_rate), rx_tput, dtoh32(sta->rx_rate));
	}
